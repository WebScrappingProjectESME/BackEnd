...
export default class Generator {
    ...
    weekLowFreq = (player) => (x) => R.product([this.amplitudeCoeff,player,Math.sin(R.add(R.product([this.frequency,2,Math.PI,x]),this.dephasage))]); //product
    weekHighFreq = (player) => (x) => R.product([this.amplitudeCoeff,this.amplitudeCoeff,player,Math.sin(R.add(R.product([2,this.frequency,2,Math.PI,x]),R.product([2,this.dephasage])))]);
    monthFreq = (player) => (x) => R.product([this.amplitudeCoeff,player,Math.sin(R.product([this.frequency,2,Math.PI,x]))]);
    yearsFreq = () => 0;
    ...
    generateListOfPlayers = async (AppId) => {
        ...
        return listOfHour.map(R.pipe(R.juxt([this.weekLowFreq(currentPlayer), this.weekHighFreq(currentPlayer), this.monthFreq(currentPlayer), this.getRandomNoise(currentPlayer)]), R.sum, R.add(currentPlayer), R.add(this.yearsFreq()), Math.floor));
    }
    ...
    getRandomListOfSale() {
        ...
        const listOfSales = R.times(this.getOneRandomObjectSale, randomNumberOfSale);
